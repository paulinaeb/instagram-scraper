...
class Instagram:
    ...
    def __init__(self, sleep_between_requests=0, min_sleep=0, max_sleep=0):
    ...
    def get_medias_by_user_id(self, id, count=12, max_id=''):
        ...
        while index < count and is_more_available:
            ...
            print(f'Sending Request to get {index} posts ({self.number_of_requests} requests so far)...\n');
            response = self.__req.get(
    ...
    def get_media_likes_by_code(self, code, count=10, max_id=None):
        ...
        while (has_previous and index < count):
            ...
            time.sleep(random.uniform(self.min_sleep, self.max_sleep))
            self.number_of_requests += 1
            print(f'Sending request for {number_of_likes_to_receive} likers ({self.number_of_requests} requests so far)...');
            ...
            if not response.status_code == Instagram.HTTP_OK:
                # print('BAD RESPONSE IN GETTING LIKERS\n', json.dumps(response.json(), indent=2))
                print('BAD RESPONSE\n',response.json())
                raise InstagramException.default(response.text,response.status_code)
    ...
    def get_media_comments_by_code(self, code, count=10, max_id=''):
        ...
        while has_previous and index < count:
            ...
            time.sleep(random.uniform(self.min_sleep, self.max_sleep))
            self.number_of_requests += 1
            print(f'Sending request for {number_of_comments_to_receive} comments ({self.number_of_requests} requests so far)...');
            ...
            if not response.status_code == Instagram.HTTP_OK:
                print('BAD RESPONSE IN GETTING COMMENTS\n', response.json())
                raise InstagramException.default(response.text,
                                                 response.status_code)
    ...
    def get_account(self, username):
        ...
        time.sleep(self.sleep_between_requests)
        self.number_of_requests += 1
        print(f'Sending request to get user info ({self.number_of_requests} requests so far)...')
